...
class User extends Admin 
{
    ...
    public function listItems($params, $options) 
    {
        ...
        {
            $query = self::from('hgcms_user as u')->select('u.id','u.name', 'u.username','u.status', 'u.thumb','u.email','u.phone', 'g.name as level')
    ...
    public function getItems($params,$options) 
    {
        ...
        if($options['task'] === 'auth-login') 
        {
            ...
            if($user && Hash::check($params['password'], $user->password)) {
                $result = self::from('hgcms_user as u')
                            ->select('u.id','u.name', 'u.username','u.status', 'u.thumb','u.email','u.phone', 'g.name as level','g.permision as permision')
                            ->leftJoin('hgcms_group as g', 'u.group_id','=','g.id')
                            ->where('u.id',$user->id)
                            ->first();
            }
        ...
        {
            $result = self::from('hgcms_user as u')
                            ->select('u.id','u.name', 'u.username','u.status', 'u.thumb','u.content','u.email','u.phone', 'g.name as level')
                            ->leftJoin('hgcms_group as g', 'u.group_id','=','g.id')
